-- Load URLs from file into a table (at script level, available to all threads)
local urls = {}
local file = io.open("short_urls.txt", "r")
if file then
    ...
    print("ERROR: Could not open short_urls.txt")
end
...
function init(args)
    ...
    -- Usage: wrk -t12 -c400 -d60s -s lookup_load.lua http://localhost:8080 -- 12
    ...
    if total_threads == nil or total_threads == 0 then
        ...
        print("Usage: wrk -t12 -c400 -d60s -s lookup_load.lua http://localhost:8080 -- 12")
        total_threads = 1
    end
...
function request()
    ...
    -- If we've exhausted our URLs, wrap (will hit same URLs again, which is fine for lookups)
    if index > #urls then
        index = thread_id + 1  -- Restart from first URL for this thread
        request_count = 1
    end
    
    local short = urls[index]
    ...
    return wrk.format("GET", "/api/v1/tinyurl/" .. short)
end
